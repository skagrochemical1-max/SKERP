...
// Mapper: React -> DB
export const mapReactToDbFormulation = (reactForm, inventoryItems = []) => {
  const norm = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const parsedProdId = parseInt(reactForm.productId, 10);

  return {
    product_id: isNaN(parsedProdId) ? null : parsedProdId,
    ...
    ingredients: (reactForm.ingredients || []).map(ing => {
      let pId = parseInt(ing.productId, 10);
      if (isNaN(pId) || !pId) {
        pId = null;
        if (ing.name && inventoryItems.length > 0) {
          const ingNorm = norm(ing.name);
          const match = inventoryItems.find(inv => norm(inv.name) === ingNorm);
...
export const FormulationProvider = ({ children }) => {
  ...
  const addFormulation = async (formulation) => {
    try {
      const dbPayload = mapReactToDbFormulation(formulation, inventoryItems);
  ...
  const updateFormulation = async (id, updatedFormulation) => {
    try {
      const dbPayload = mapReactToDbFormulation(updatedFormulation, inventoryItems);
