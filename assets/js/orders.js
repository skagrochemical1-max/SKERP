...
async function saveOrder() {
  ...
  try {
    ...
    const executeSave = async (allowBackorder = false) => {
      // Sync latest live DOM values from form table to orderItems array safely
      document.querySelectorAll('#order-items-tbody tr').forEach((row, idx) => {
        if (orderItems[idx]) {
          ...
          if (packSel && packSel.value && packSel.value.trim() !== '') {
            orderItems[idx].packaging_size = packSel.value.trim();
          } else if (packInput && packInput.value && packInput.value.trim() !== '') {
            orderItems[idx].packaging_size = packInput.value.trim();
          }
          const bottleSel = row.querySelector('.order-bottle-select');
          if (bottleSel && bottleSel.value && bottleSel.value.trim() !== '') {
            orderItems[idx].bottle_inventory_id = parseInt(bottleSel.value);
          }
          const qtyInp = row.querySelector('.item-qty-input');
          if (qtyInp && qtyInp.value !== '') {
            const parsedQty = parseFloat(qtyInp.value);
            if (!isNaN(parsedQty)) orderItems[idx].quantity = parsedQty;
          }
          const priceInp = row.querySelector('.item-price-input');
          if (priceInp && priceInp.value !== '') {
            const parsedPrice = parseFloat(priceInp.value);
            if (!isNaN(parsedPrice)) orderItems[idx].unit_price = parsedPrice;
          }
      ...
      // Normalized alphanumeric helper
      ...
      for (const item of orderItems) {
        ...
        // Ensure pack size fallback if empty
