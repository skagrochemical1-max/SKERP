...
(function() {
  ...
  function getDbClient() {
    ...
    if (window.supabase) {
      const url = 'https://uepuzyvdfyylztyecpug.supabase.co';
      const key = 'sb_publishable__UaVFmqPrT2UVq_brpCeUQ_p4r63kcz';
      ...
      try {
        ...
        if (path === 'formulations' && method === 'POST') {
          ...
          if (body.ingredients && body.ingredients.length > 0) {
            // Fetch inventory items to auto-match product_id if not explicitly provided
            const { data: invItems } = await client.from('inventory_items').select('id, name');
            ...
            const ingPayload = body.ingredients.map(ing => {
              ...
              if (!pId && invItems && pName) {
                const pNorm = norm(pName);
                const match = invItems.find(i => norm(i.name) === pNorm || (pNorm.length > 3 && (norm(i.name).includes(pNorm) || pNorm.includes(norm(i.name)))));
